...
const Weekand = () => {
  const { allschedule, isActive } = useTracker();
  ...
  function getHour() {
    const totalMinutes =
      allschedule?.reduce((acc, currentValue) => {
        const [h1, m1] = currentValue.start.split(":").map(Number);
        const [h2, m2] = currentValue.end.split(":").map(Number);
        const itemMinutes = h2 * 60 + m2 - (h1 * 60 + m1);
        const positiveMinutes =
          itemMinutes < 0 ? itemMinutes + 24 * 60 : itemMinutes;

        return acc + positiveMinutes;
      }, 0) || 0; // Default to 0 if allschedule is empty
  ...
  <>
    <div className="lg:w-[30%] lg:mx-auto mx-2 lg:h-[10rem] h-[8rem] border shadow-lg border-gray-300 rounded-2xl flex flex-col justify-center lg:gap-16 gap-4 p-10">
      ...
      <p className="text-xl text-gray-600 dark:text-gray-300">
        0h 0m / {gethour.hour}h {gethour.min}m Complete
      </p>
    </div>
  </>
)}
...
{(isActive === "daily" || isActive === "weekly") && (
  <>
    <div className="lg:w-[30%] lg:mx-auto mx-2 lg:h-[10rem] h-[8rem] border shadow-lg border-gray-300 rounded-2xl flex flex-col justify-center lg:gap-16 gap-4 p-10">
      ...
      <p className="text-xl text-gray-600 dark:text-gray-300">
        0h 0m / {Math.floor((gethour.totalMin * 7) / 60)}h{" "}
